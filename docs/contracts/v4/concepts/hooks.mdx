...
This page describes hooks in the current `FewToken + Uniswap v4` integration stack. It should be read as integration documentation, not as documentation for a separate native v4 AMM.
...
For the deployed FewToken pools, exact hook permissions, source provenance, ABIs, and integration vectors, see
[FewToken Liquidity and Aggregator Integration](/contracts/v4/guides/fewtoken-liquidity-aggregation#few-wrapper-hooks).

## Key Concepts

### Pool-Specific Hooks

- Each liquidity pool in the v4 integration environment can have its own hook contract attached to it. Hooks are optional for v4 pools.
...
The v4 hook model provides a set of core hook functions that can be implemented by developers. Developers do not have to implement every hook, and can mix and match them as needed.

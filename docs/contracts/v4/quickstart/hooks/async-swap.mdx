...
One feature enabled by [custom accounting](/contracts/v4/guides/custom-accounting) is AsyncSwap. This feature allows hook developers to replace the default v4 swap logic.

This means developers can replace the v4 core logic for how to handle swaps. Two emergent use-cases are possible with custom accounting:
...
contract AsyncSwapHook is BaseHook {
        ...
        returns (bytes4, BeforeSwapDelta, uint24)
    {
        ...
        if (params.amountSpecified < 0) {
            // take the input token so the default swap path is skipped...

...
When building against this v4 integration stack, you will often need to read pool state for both onchain and offchain use cases. Onchain contracts can directly invoke the [**StateLibrary**](https://github.com/Uniswap/v4-core/blob/main/src/libraries/StateLibrary.sol) to execute these reads during transactions, but offchain systems such as frontends or analytics services require a deployed contract with view functions. This is where [**StateView**](https://github.com/RingProtocol/v4-periphery/blob/main/src/lens/StateView.sol) comes in.
...
- If you pass an invalid `poolId` (typically a [`bytes32`](https://github.com/Uniswap/v4-core/blob/main/src/types/PoolId.sol#L6) in this v4 integration stack), the call may revert or return unexpected data.

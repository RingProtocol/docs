...
In the current v4 integration stack, when a hook is triggered, `msg.sender` is always the `PoolManager` contract, not the EOA or smart-account that initiated the swap. This behavior occurs because the `PoolManager` acts as an intermediary, executing the swap logic on behalf of the user.
...
> [Refer to Building your first hook](/contracts/v4/guides/hooks/your-first-hook#setting-things-up)

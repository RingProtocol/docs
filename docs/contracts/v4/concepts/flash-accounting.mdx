...
This page describes flash accounting in the current `FewToken + Uniswap v4` integration environment. It should be read as technical integration guidance, not as documentation for a separate native v4 AMM.

In older pool-per-contract AMM designs, every swap - including multi-hop swaps - transfers tokens between pool contracts
...
With the singleton architecture, a better design was possible and is referred to as _Flash Accounting_. The design became practical with gas efficiencies of [Transient Storage](https://eips.ethereum.org/EIPS/eip-1153). _Flash Accounting_ further reduces the gas cost of trades that cross multiple pools and supports more complex integrations with this v4 stack.
...
![Flash Accounting Step 1](./images/Ring_V4_Flash_Accounting_Step_1.png)
![Flash Accounting Step 2](./images/Ring_V4_Flash_Accounting_Step_2.png)
...
![Flash Accounting Locking Mechanism Step 1](./images/Ring_V4_Locking_Mechanism_Step_1.png)

2. Implement `unlockCallback` and proceed with any desired pool actions
![Flash Accounting Locking Mechanism Step 2](./images/Ring_V4_Locking_Mechanism_Step_2.png)

3. The actual token transfer happens at the end and the *delta* should be resolved
![Flash Accounting Locking Mechanism Step 3](./images/Ring_V4_Locking_Mechanism_Step_3.png)
...
![Multi-hop swaps with flash accounting](./images/Ring_V4_Multihop_Swaps.png)
...
### Earlier pool-per-contract flow

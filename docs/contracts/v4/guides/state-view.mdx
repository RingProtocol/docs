...
When building on **Ring v4**, you will often need to read pool state for both onchain and offchain use cases. Onchain contracts can directly invoke the [**StateLibrary**](https://github.com/RingProtocol/v4-core/blob/main/src/libraries/StateLibrary.sol) to execute these reads during transactions, but offchain systems—such as frontends or analytics services—require a deployed contract with view functions. This is where [**StateView**](https://github.com/RingProtocol/v4-periphery/blob/main/src/lens/StateView.sol) comes in.
...
If you’re familiar with [Reading Pool State](/contracts/v4/guides/read-pool-state), you already know that Ring v4 uses **extsload** for efficient data access. For onchain usage, we rely on **StateLibrary** within contracts. However, offchain clients cannot rely on an onchain library for state reads. 
...
- If you pass an invalid `poolId` (typically a [`bytes32`](https://github.com/RingProtocol/v4-core/blob/main/src/types/PoolId.sol#L6) in Ring v4), the call may revert or return unexpected data.
...
- In **Ring v4**, a `poolId` is typically a `bytes32` that is derived by calling 
...
**StateView** is a powerful and efficient way to read Ring v4 pool data offchain. By separating onchain logic (using `StateLibrary`) and offchain reads (using `StateView`), Ring ensures the best developer experience for both contexts.

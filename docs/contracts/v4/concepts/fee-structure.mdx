...
This page describes fee behavior in the current `FewToken + Uniswap v4` integration environment. It should be read as technical integration guidance, not as documentation for a separate native v4 AMM.

In this v4 integration stack, there are three main types of fees to understand:

- **LP Fee**: Fees allocated to liquidity providers
...
- **Unlimited fee options** in v4-style pools
- Integrators should confirm the selected LP fee from the pool configuration instead of assuming fixed fee tiers
...
In this v4 integration stack, the swap fee is the effective total fee after applying both protocol and LP fees sequentially.
...
**Impact on LP Fee Share:**

Note that this sequential application means introducing or increasing protocol fees will reduce the LP fee share even if swap volume remains constant, since LP fees are calculated on a smaller base amount.

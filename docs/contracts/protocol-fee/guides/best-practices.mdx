...
Below, please find a list of best practices when interacting with the Ring Protocol Fee system.
...
## Collect Ring v3 Fees

For further reading, please see [Read Asset Balance](./read-asset-balance)

In the Ring Protocol Fee system, Ring v3 fees *must be collected* to the `AssetSink` before they are eligible for release.

:::tip
We recommend that integrators check and collect Ring v3 fees before calling `Firepit.release()`, to ensure the maximum assets are released.

Collection is available via the active fee adapter contracts documented in the technical reference.
...
The Ring Protocol Fee system allows for an *updatable* [`Firepit.threshold()`](../technical-reference/IResourceManager#threshold)
...
Because the Ring Protocol Fee system can release native tokens (Ether), recipients of the tokens should be `payable`

## Pricing Ring v2 ERC-20 Tokens

For further reading, please see [Read Asset Balance](./read-asset-balance#ring-v2-pool-protocol-fees)

Ring v2 Protocol Fees are automatically pushed to the `AssetSink` contract, but are represented as ERC-20 LP tokens. These ERC-20 LP tokens represent a combination of `token0` and `token1`

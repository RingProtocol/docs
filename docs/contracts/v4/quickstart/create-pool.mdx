...
This page describes how to create pools in the current `FewToken + Uniswap v4` integration environment. It should not be read as evidence that Ring operates a separate native v4 AMM.

Creating a pool in this v4 integration environment is permissionless and enables the trading of an asset. Uniswap v4-style infrastructure is a useful destination for market creation due to its:
...
For more information, developers should see [Uniswap v4 Integration Overview](/contracts/v4/overview)
...
Developing against the current v4 integration stack _requires [foundry](https://book.getfoundry.sh)_
...
To initialize a v4 pool _without initial liquidity_, developers should call [`PoolManager.initialize()`](/contracts/v4/reference/core/interfaces/IPoolManager#initialize)
...
As a reference, common concentrated-liquidity fee configurations include:
...
The v4 [PositionManager](/contracts/v4/reference/periphery/PositionManager) supports atomic creation of a pool and initial liquidity using [_multicall_](/contracts/v4/reference/periphery/base/Multicall_v4). Developers can create a trading pool, with liquidity, in a single transaction:
...
### 3. Encode the [`initializePool`](/contracts/v4/reference/periphery/base/PoolInitializer_v4) parameters
Pools are initialized with a starting price
```solidity
...
Ring-specific end-to-end scripts should be published only after the Ring team has finalized the supported hook and pool
creation workflow.

...
This page describes the `PoolManager` in the current `FewToken + Uniswap v4` integration environment. It should be read as technical integration guidance, not as documentation for a separate native v4 AMM.

In older concentrated-liquidity designs, each liquidity pool was represented by a separate smart contract deployed through a factory contract. While this approach provided flexibility, it also led to increased gas costs for pool creation and multi-hop swaps.
...
- Reduce gas costs compared to older factory-based pool designs

...
| pairCount         | Int        | amount of pairs created by the Ring factory                  |
...
Transaction entities are created for each Ethereum transaction that contains an interaction within Ring contracts. This subgraph tracks Mint, Burn, and Swap events on the Ring core contracts. Each transaction contains 3 arrays, and at least one of these arrays has a length of 1.
...
Mint entities are created for every emitted Mint event on the Ring core contracts. The Mint entity stores key data about the event like token amounts, who sent the transaction, who received the liquidity, and more. This entity can be used to track liquidity provisions on pairs.
...
Burn entities are created for every emitted Burn event on the Ring core contracts. The Burn entity stores key data about the event like token amounts, who burned LP tokens, who received tokens, and more. This entity can be used to track liquidity removals on pairs.
...
| mostLiquidTokens  | [TokenDayData!] | tokens with most liquidity in Ring                                            |

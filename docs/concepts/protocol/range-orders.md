...
:::note
Range orders are a concentrated-liquidity concept. In the current Ring stack, this page should be read as a conceptual explanation for external concentrated-liquidity integration environments rather than as documentation for a native Ring v3 or Ring v4 AMM.
:::
...
In a concentrated-liquidity AMM, one can approximate a limit order by providing a single asset as liquidity within a specific range. Like traditional limit orders, range orders may be set with the expectation they will execute at some point in the future, with the target asset available for withdrawal after the spot price has crossed the full range of the order.

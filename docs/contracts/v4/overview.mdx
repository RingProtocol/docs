...
Ring v4 inherits all of the capital efficiency gains of Ring v3, but provides flexibility via *hooks* and gas optimizations across the entire lifecycle.

For additional information, see the [Ring v4 whitepaper](https://app.ring.exchange/whitepaper-v4.pdf)
...
pool creation, liquidity addition and removal, swapping, and donations. Hooks are deployed contracts, and are called by the Ring v4 PoolManager,
...
Ring v4 supports dynamic fees, allowing pools to adjust their fees up or down. While other AMMs may have hard-coded logic for dynamic fees,
...
Ring v4 supports native token assets (Ether), without the need to wrap/unwrap the native token to Wrapped Ether (WETH9).

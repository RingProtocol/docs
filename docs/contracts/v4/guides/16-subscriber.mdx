...
Subscribers are a v4 mechanism that allows external contracts (aka Subscribers) to be notified by the PositionManager whenever specific events occur (e.g. liquidity changes or burns). It’s especially useful for designing **liquidity incentive** systems or reward programs.

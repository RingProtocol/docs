...
Swaps are the most common interaction with the Ring protocol. The following example shows you how to implement a single-path swap contract that uses two functions that you create:
...
our contract is a contract itself and not an extension of the caller (us); we must also approve the Ring protocol router contract to use the tokens that our contract will be in possession of after they have been withdrawn from the calling address (us).

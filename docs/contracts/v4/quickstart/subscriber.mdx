...
This page describes subscriber contracts in the current `FewToken + Uniswap v4` integration environment. It should be read as integration guidance, not as documentation for a separate native v4 AMM.

For developers looking to support custom external position tracking, Subscriber contracts can be used to receive notifications about position modifications or transfers.

## Guide

### 1. Implement the [`ISubscriber`](https://github.com/RingProtocol/v4-periphery/blob/main/src/interfaces/ISubscriber.sol) interface

Can also refer to [MockSubscriber](https://github.com/RingProtocol/v4-periphery/blob/main/test/mocks/MockSubscriber.sol) for an actual implementation example.
...
To prevent gas griefing during unsubscription, this v4 integration stack uses a fixed [`unsubscribeGasLimit`](/contracts/v4/reference/periphery/interfaces/INotifier#unsubscribegaslimit) when calling a subscriber’s `notifyUnsubscribe()` function.
...
From [`_unsubscribe()`](https://github.com/RingProtocol/v4-periphery/blob/main/src/base/Notifier.sol#L80) on `Notifier`:
```solidity
